...
export default class ListSort extends React.Component {
    ...
    const childStyle = this.children.map((item, ii) => {
      ...
      if (cItem) {
        ...
        const parentHeight =
          ...
          parseFloat(
            getComputedStyle(item.parentNode).getPropertyValue("padding-bottom")
          );
        const parentWidth =
          item.parentNode.clientWidth -
          parseFloat(
            getComputedStyle(item.parentNode).getPropertyValue("padding-right")
          );
      ...
      (item, ii) =>
        (i === ii &&
          (!this.props.dragClassName
            ? { scale: 1.2, boxShadow: "0 10px 10px rgba(0,0,0,0.15)" }
            : null)) ||
        null
    );
    this.index = i;
  ...
  onMouseMove = (e) => {
    ...
    if (this.props.animType === "x") {
      childStyle[this.index].left =
        this.mouseXY.x - this.mouseXY.startX + this.mouseXY.left;
    } else {
      childStyle[this.index].top =
        this.mouseXY.y - this.mouseXY.startY + this.mouseXY.top;
      this.swapIndex =
        childStyle[this.index].top < this.childStyle[this.index].top
          ? 0
          : this.index;
      this.swapIndex =
        childStyle[this.index].top >
      ...
      animation = animation.map((item, i) => {
        ...
        if (this.index < this.swapIndex) {
          ...
          if (
            (i > this.swapIndex || this.swapIndex === this.index) &&
            i !== this.index
          ) {
          ...
          if (
            ...
            height =
              this.childStyle[this.index].height +
              this.childStyle[this.index].marginHeight;
  ...
  render() {
    const childrenToRender = toArrayChildren(this.state.children).map(
      this.getChildren
    );
