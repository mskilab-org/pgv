...
function* launchApplication() {
  ...
  if (responseSettings && responseDatafiles) {
    ...
    let plots = [{type: "genes", source: `/genes/${selectedCoordinate}.json`}, ...datafile.plots.map(d => {return {...d, source: `data/${file}/${d.source}`}})];
    yield axios.all(plots.filter((d,i) => ["genes", "genome"].includes(d.type)).map(d => axios.get(d.source))).then(axios.spread((...responses) => {
      responses.forEach((d,i) => plots.filter((d,i) => ["genes", "genome"].includes(d.type))[i].data = d.data);
    })).catch(errors => {
      console.log("got errors on loading dependencies", errors)
    });

    yield all([...plots.filter((d,i) => ["barplot", "scatterplot"].includes(d.type)).map(x => call(fetchArrowData, x))]);
